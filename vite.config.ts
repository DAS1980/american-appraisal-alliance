 ...
 * 5. Production domain fallback: https://americanappraisalalliance.com
 */
function getSiteDomain(manifestDomain: string): string {
  ...
  return 'https://americanappraisalalliance.com';
}
...
function generateHeadContent(
  ...
  // Robots — always block indexing for subdomain-hosted sites
  const isSubdomain = baseUrl.includes('.builder.') && baseUrl.includes('searchatlas.com');
  if (isSubdomain) {
